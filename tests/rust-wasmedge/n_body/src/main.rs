...
pub fn offset_momentum(bodies: &mut [SBody; N_BODIES]) {
...
pub fn energy(bodies: &[SBody; N_BODIES]) -> f64 {
    let mut e = 0.;
    for i in 0..N_BODIES {
        let bi = &bodies[i];
...
pub fn advance(bodies: &mut [SBody; N_BODIES], dt: f64) {
...
async fn n_body(req: Request<Body>) -> Result<Response<Body>, HyperError> {
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  loop {
    ...
    tokio::task::spawn(async move {
        ...
        //    }
        if let Err(err) = http.serve_connection(stream, service_fn(n_body)).await {
            println!("Error serving connection: {:?}", err);
        }
    });
  }
